...
const MultipleImageUploader: React.FC<MultipleImageUploaderProps> = ({
  ...
  // Update preview when props change - use JSON comparison to avoid infinite loops
  React.useEffect(() => {
    const currentImagesString = JSON.stringify(currentImages);
    const currentImagesWithLabelsString = JSON.stringify(currentImagesWithLabels);
    
    if (enableLabels) {
      const previewString = JSON.stringify(previewImagesWithLabels);
      if (previewString !== currentImagesWithLabelsString) {
        setPreviewImagesWithLabels(currentImagesWithLabels);
      }
    } else {
      const previewString = JSON.stringify(previewImages);
      if (previewString !== currentImagesString) {
        setPreviewImages(currentImages);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(currentImages), JSON.stringify(currentImagesWithLabels), enableLabels]);
